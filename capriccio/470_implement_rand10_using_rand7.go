...
func rand10() int {
	...
	// 对于 41 ~ 49，比较简单的处理方式是直接抛弃，直到获取的数字是 1 到 40 为止（拒绝采样）。每次运行程序会生成 1 到 40 的概率 p 为 40/49，根据独立事件的期望公式 Ex = n * p, 程序运行的期望运行次数为 n 为1.225，每次运行会调用 2 次 rand7，所以 rand7 的调用期望为 1.225 * 2。
	if c <= 40 {
